...
namespace VerdureCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Testimonials', 'verdure-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'verdure-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'verdure-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'verdure-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'box_color',
							'heading'    => esc_html__( 'Content Box Color', 'verdure-core' ),
						...
						array(
							...
							'value'       => array_flip( verdure_mikado_get_yes_no_select_array() ),
							'group'       => esc_html__( 'Slider Settings', 'verdure-core' )
						),
						array(
							'type'        => 'dropdown',
	...
	public function render( $atts, $content = null ) {
		...
		$html = '<div class="mkdf-testimonials-holder ' . $holder_classes . ' clearfix">';
			$html .= '<div class="mkdf-testimonials mkdf-owl-slider" ' . verdure_mikado_get_inline_attrs( $data_attr ) . '>';
			...
			if ( $query_results->have_posts() ):
				while ( $query_results->have_posts() ) : $query_results->the_post();
					...
					$html .= verdure_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
				
				endwhile;
			else:
				$html .= esc_html__( 'Sorry, no posts matched your criteria.', 'verdure-core' );
			endif;
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'verdure-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'verdure-core' ) . ': ' . $testimonials_category_title;
				}
