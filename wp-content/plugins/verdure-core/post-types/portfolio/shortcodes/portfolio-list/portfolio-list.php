...
class PortfolioList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Mikado Portfolio List', 'verdure-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Portfolio List Template', 'verdure-core' ),
							'value'       => array(
								esc_html__( 'Gallery', 'verdure-core' ) => 'gallery',
								esc_html__( 'Masonry', 'verdure-core' ) => 'masonry'
							),
							'save_always' => true,
							'admin_label' => true
						),
						...
						array(
							...
							'value'       => array_flip( verdure_mikado_get_space_between_items_array(true) ),
							'save_always' => true
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'number_of_items',
							'heading'     => esc_html__( 'Number of Portfolios Per Page', 'verdure-core' ),
							'description' => esc_html__( 'Set number of items for your portfolio list. Enter -1 to show all.', 'verdure-core' ),
							'value'       => '-1'
						),
						array(
							'type'        => 'dropdown',
							...
							'description' => esc_html__( 'Set image proportions for your portfolio list.', 'verdure-core' ),
							'dependency'  => array( 'element' => 'type', 'value' => array( 'gallery' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'enable_fixed_proportions',
							'heading'     => esc_html__( 'Enable Fixed Image Proportions', 'verdure-core' ),
							'value'       => array_flip( verdure_mikado_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Set predefined image proportions for your masonry portfolio list. This option will apply image proportions you set in Portfolio Single page - dimensions for masonry option.', 'verdure-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Pagination Type', 'verdure-core' ),
							'value'      => array(
								esc_html__( 'None', 'verdure-core' )            => 'no-pagination',
								esc_html__( 'Standard', 'verdure-core' )        => 'standard',
								esc_html__( 'Load More', 'verdure-core' )       => 'load-more',
								esc_html__( 'Infinite Scroll', 'verdure-core' ) => 'infinite-scroll'
							),
							'group'      => esc_html__( 'Additional Features', 'verdure-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'load_more_top_margin',
							'heading'    => esc_html__( 'Load More Top Margin (px or %)', 'verdure-core' ),
						...
						array(
							'type'       => 'dropdown',
							'param_name' => 'filter',
							'heading'    => esc_html__( 'Enable Category Filter', 'verdure-core' ),
							'value'      => array_flip( verdure_mikado_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Additional Features', 'verdure-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Order By', 'verdure-core' ),
							'value'      => array(
								esc_html__( 'Name', 'verdure-core' )  => 'name',
								esc_html__( 'Count', 'verdure-core' ) => 'count',
								esc_html__( 'Id', 'verdure-core' )    => 'id',
								esc_html__( 'Slug', 'verdure-core' )  => 'slug'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Filter Text Transform', 'verdure-core' ),
							'value'      => array_flip( verdure_mikado_get_text_transform_array( true ) ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'verdure-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Bottom Margin (px or %)', 'verdure-core' ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'verdure-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Article Animation', 'verdure-core' ),
							'value'       => array_flip( verdure_mikado_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable appears animation for your portfolio list items', 'verdure-core' ),
							'group'       => esc_html__( 'Additional Features', 'verdure-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$additional_params['holder_data']          = verdure_mikado_get_holder_data_for_cpt( $params, $additional_params );
		...
		$html = verdure_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
		...
		if ( $params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes' ) {
			...
			switch ( $fixed_image_size ) {
				...
				case 'large-width-height':
					$thumb_size = 'verdure_mikado_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
	...
	public function getStandardContentStyles( $params ) {
		...
		if ( ! empty( $margin_top ) ) {
			if ( verdure_mikado_string_ends_with( $margin_top, '%' ) || verdure_mikado_string_ends_with( $margin_top, 'px' ) ) {
		...
		if ( ! empty( $margin_bottom ) ) {
			if ( verdure_mikado_string_ends_with( $margin_bottom, '%' ) || verdure_mikado_string_ends_with( $margin_bottom, 'px' ) ) {
	...
	public function getLoadMoreStyles( $params ) {
		...
		if ( ! empty( $params['load_more_top_margin'] ) ) {
			...
			if ( verdure_mikado_string_ends_with( $margin, '%' ) || verdure_mikado_string_ends_with( $margin, 'px' ) ) {
				$styles[] = 'margin-top: ' . $margin;
			} else {
				$styles[] = 'margin-top: ' . verdure_mikado_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( verdure_mikado_string_ends_with( $margin, '%' ) || verdure_mikado_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . verdure_mikado_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getItemLink() {
		...
		return apply_filters( 'verdure_mikado_portfolio_external_link', $portfolio_link );
	}
	...
	public function getItemLinkTarget() {
		...
		return apply_filters( 'verdure_mikado_portfolio_external_link_target', $portfolio_link_target );
	}
