<?php
namespace VerdureCore\CPT\Shortcodes\Separator;
...
class Separator implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                    => esc_html__( 'Mikado Separator', 'verdure-core' ),
					'base'                    => $this->base,
					...
					'params'                  => array_merge(
						array(
							...
							array(
								...
								'value'      => array(
									esc_html__( 'Normal', 'verdure-core' )     => 'normal',
									esc_html__( 'Full Width', 'verdure-core' ) => 'full-width'
								)
							),
							array(
								'type'       => 'dropdown',
								'param_name' => 'position',
								'heading'    => esc_html__( 'Position', 'verdure-core' ),
								'value'      => array(
							...
							array(
								...
								'value'      => array(
									esc_html__( 'None', 'verdure-core' )     => 'no',
									esc_html__( 'Icon from Icon Pack', 'verdure-core' ) => 'icon-pack',
									esc_html__( 'Custom Icon', 'verdure-core' ) => 'custom-icon',
								),
								'dependency' => array( 'element' => 'type', 'value' => array( 'normal' ) )
							),
						),
						verdure_mikado_icon_collections()->getVCParamsArray(array('element' => 'enable_icon', 'value' => 'icon-pack'), '', true),
						array(
							array(
								'type'       => 'textfield',
								'param_name' => 'icon_size',
								'heading'    => esc_html__( 'Icon Size (px)', 'verdure-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Icon', 'verdure-core' ),
								'dependency' => array( 'element' => 'enable_icon', 'value' => array( 'custom-icon' ) )
							),
							array(
								'type'       => 'dropdown',
								'param_name' => 'icon_position',
								'heading'    => esc_html__( 'Icon Position', 'verdure-core' ),
								'value'      => array(
									esc_html__( 'Center', 'verdure-core' ) => 'center',
									esc_html__( 'Left', 'verdure-core' )   => 'left',
									esc_html__( 'Right', 'verdure-core' )  => 'right'
							...
							array(
								...
								'heading'    => esc_html__( 'Icon Left/Right Margin (px)', 'verdure-core' ),
								'dependency' => array( 'element' => 'enable_icon', 'value' => array( 'icon-pack', 'custom-icon' ) )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'value'       => array(
									...
									esc_html__( 'Dashed', 'verdure-core' )  => 'dashed',
									esc_html__( 'Solid', 'verdure-core' )   => 'solid',
									esc_html__( 'Dotted', 'verdure-core' )  => 'dotted'
								),
								'save_always' => true
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'width',
								'heading'    => esc_html__( 'Width (px or %)', 'verdure-core' ),
								'dependency' => array( 'element' => 'type', 'value' => array( 'normal' ) )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'thickness',
								'heading'    => esc_html__( 'Thickness (px)', 'verdure-core' )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'top_margin',
								'heading'    => esc_html__( 'Top Margin (px or %)', 'verdure-core' )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'bottom_margin',
								'heading'    => esc_html__( 'Bottom Margin (px or %)', 'verdure-core' )
							)
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$args = array_merge( $args, verdure_mikado_icon_collections()->getShortcodeParams() );
		...
		$html = verdure_core_get_shortcode_module_template_part( 'templates/separator-template', 'separator', '', $params );
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['top_margin'] !== '' ) {
			if ( verdure_mikado_string_ends_with( $params['top_margin'], '%' ) || verdure_mikado_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . verdure_mikado_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( verdure_mikado_string_ends_with( $params['bottom_margin'], '%' ) || verdure_mikado_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . verdure_mikado_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
	...
	private function getSeparatorStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			...
			if ( verdure_mikado_string_ends_with( $params['width'], '%' ) ) {
				$width_suffix = '%';
			}
		...
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . verdure_mikado_filter_px( $params['thickness'] ) . 'px';
		}
	...
	private function getIconHolderStyle( $params ) {
		...
		if ($params['icon_margin'] !== '') {
			if ($params['icon_position'] == 'center') {
				$holder_style[] = 'margin: 0 '.verdure_mikado_filter_px($params['icon_margin']).'px';
			} elseif ($params['icon_position'] == 'left') {
				$holder_style[] = 'margin-right: '.verdure_mikado_filter_px($params['icon_margin']).'px';
			} else {
				$holder_style[] = 'margin-left: '.verdure_mikado_filter_px($params['icon_margin']).'px';
			}
		}
	...
	private function getIconHTML( $params ) {
		...
		if ( $params['enable_icon'] == 'custom-icon' && $params['custom_icon'] !== '' ) {
			...
			if ( $params['icon_size'] !== '' ) {
				$icon_parameters['custom_size'] = verdure_mikado_filter_px( $params['icon_size'] ) . 'px';
			}
			...
			$icon = verdure_mikado_execute_shortcode('mkdf_icon', $icon_parameters);
		}
