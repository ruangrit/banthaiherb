...
class VerdureCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( VERDURE_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		...
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'verdure-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'verdure-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		$this->message = esc_html__( 'Widgets imported successfully', 'verdure-core' );
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'verdure-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'mkdf_options_verdure', $options );
		$this->message = esc_html__( 'Options imported successfully', 'verdure-core' );
	}
	...
	function mkdf_admin_import() {
		if ( verdure_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				esc_html__( 'Mikado Options - Mikado Import', 'verdure-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'verdure-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
				'verdure_mikado_theme_menu' . $slug,                  // The ID used to bind submenu items to this menu
				array( $verdure_mikado_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'verdure_mikado_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'verdure_mikado_enqueue_admin_styles' );
		}
	}
	...
	function mkdf_update_meta_fields_after_import($folder){
		...
		$images_field_array = verdure_mikado_meta_boxes()->getOptionsByType('image');
	...
	function mkdf_update_options_after_import( $folder ) {
		...
		$global_options    = get_option( 'mkdf_options_verdure' );
		$new_global_values = str_replace( $demo_url, $url, $global_options );
		update_option( 'mkdf_options_verdure', $new_global_values );
	}

	function mkdf_import_get_demo_url( $folder ) {
		$demo_url = defined( 'MIKADO_PROFILE_SLUG' ) ? 'http://' . str_replace( '/', '', $folder ) . '.' . MIKADO_PROFILE_SLUG . '-themes.com/' : '';
