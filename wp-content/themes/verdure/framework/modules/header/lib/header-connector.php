...
namespace VerdureMikado\Modules\Header\Lib;
/**
 * Class VerdureMikadoHeaderConnector
 *
...
class VerdureMikadoHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'verdure_mikado_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'verdure_mikado_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'verdure_mikado_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'verdure_mikado_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to verdure_mikado_title_content_padding filter
	 *
	 * @return int
	 */
