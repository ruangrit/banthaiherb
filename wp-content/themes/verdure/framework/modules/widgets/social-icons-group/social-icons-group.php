...
class VerdureMikadoClassIconsGroupWidget extends VerdureMikadoWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_social_icons_group_widget',
			esc_html__( 'Mikado Social Icons Group Widget', 'verdure' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'verdure' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			verdure_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'verdure' ),
					'options' => array(
						''             => esc_html__( 'Default', 'verdure' ),
						'square-icons' => esc_html__( 'Square', 'verdure' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'verdure' ),
					'description' => esc_html__( 'This applies to the square layout', 'verdure' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'verdure' ),
						'light-skin' => esc_html__( 'Light Skin', 'verdure' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'verdure' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'verdure' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo verdure_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php verdure_mikado_inline_style( $icon_styles ) ?>
