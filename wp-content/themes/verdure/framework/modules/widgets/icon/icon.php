...
class VerdureMikadoIconWidget extends VerdureMikadoWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_icon_widget',
			esc_html__( 'Mikado Icon Widget', 'verdure' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'verdure' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			verdure_mikado_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'verdure' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? verdure_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . verdure_mikado_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		<a <?php verdure_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo verdure_mikado_get_inline_style( $holder_styles ); ?>>
