...
class VerdureMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_verdure_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_verdure_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'verdure' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'verdure' );
		}
...
if ( ! function_exists( 'verdure_mikado_create_like' ) ) {
	function verdure_mikado_create_like() {
		VerdureMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'verdure_mikado_create_like' );
}