...
/*
   Class: VerdureMikadoDashboardForm
   A class that initializes Mikado Dashboard Container
*/
class VerdureMikadoDashboardForm implements iVerdureMikadoLayoutNode, iVerdureMikadoRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render($factory) {
		...
		<form method="<?php echo esc_attr($this->form_method);?>" id="<?php echo esc_attr($this->form_id);?>" class="<?php echo esc_attr($action_class)?>" data-action="<?php echo esc_attr($this->form_action);?>">
			...
			if ( verdure_mikado_core_plugin_installed() ) {
				echo verdure_mikado_get_button_html( array(
...
/*
   Class: VerdureMikadoDashboardGroup
   A class that initializes VerdureMikado Group Field
*/
class VerdureMikadoDashboardGroup implements iVerdureMikadoLayoutNode, iVerdureMikadoRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: VerdureMikadoDashboardTitle
   A class that initializes Dashboard Title
*/
class VerdureMikadoDashboardTitle implements iVerdureMikadoRender {
	private $name;
	private $title;
...
/*
   Class: VerdureMikadoDashboardField
   A class that initializes VerdureMikado Front Field
*/

class VerdureMikadoDashboardField implements iVerdureMikadoRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class VerdureMikadoDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class VerdureMikadoDashboardFieldText extends VerdureMikadoDashboardFieldType {
...
class VerdureMikadoDashboardFieldTextArea extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class VerdureMikadoDashboardFieldImage extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="mkdf-dashboard-gallery-holder">
				...
				<div class="mkdf-dashboard-gallery-uploader">
					...
					if ( mkdf_membership_theme_installed() ) {
						echo verdure_mikado_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'verdure' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" target="_self" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'verdure' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e('Remove Media','verdure'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class VerdureMikadoDashboardFieldGallery extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class VerdureMikadoDashboardFieldSelect extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class VerdureMikadoDashboardFieldDate extends VerdureMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class VerdureMikadoDashboardFieldIcon extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = \VerdureMikadoIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \VerdureMikadoIconCollections::get_instance()->getIconCollectionsKeys();
				...
				<div class="mkdf-dashboard-item">
					...
					<div class="mkdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = \VerdureMikadoIconCollections::get_instance()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = \VerdureMikadoIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class VerdureMikadoDashboardFieldColor extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class VerdureMikadoDashboardFieldCheckBoxGroup extends VerdureMikadoDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class VerdureMikadoDashboardFieldAddress extends VerdureMikadoFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="mkdf-dashboard-field-row">
				<div class="mkdf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="mkdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'verdure' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class VerdureMikadoDashboardRepeater implements iVerdureMikadoRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'verdure' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'verdure' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'verdure' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class VerdureMikadoDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new VerdureMikadoDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new VerdureMikadoDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new VerdureMikadoDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new VerdureMikadoDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new VerdureMikadoDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new VerdureMikadoDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new VerdureMikadoDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new VerdureMikadoDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new VerdureMikadoDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new VerdureMikadoDashboardFieldAddress();
