...
class VerdureMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions         = VerdureMikadoOptions::get_instance();
		$this->mkdMetaBoxes       = VerdureMikadoMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = VerdureMikadoTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = VerdureMikadoUserOptions::get_instance();
		$this->mkdDashboardOptions = VerdureMikadoDashboardOptions::get_instance();
	}
	...
	public function setSkin( VerdureMikadoSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class VerdureMikadoSkinManager
 *
...
class VerdureMikadoSkinManager {
	...
	/**
	 * @see VerdureMikadoSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'verdure' );
...
/**
 * Class VerdureMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class VerdureMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( verdure_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo verdure_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'verdure' ) ?>';
		</script>
	<?php }
...
class VerdureMikadoOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $verdure_mikado_options ) ) {
			return $verdure_mikado_options[ $key ];
...
/*
   Class: VerdureMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class VerdureMikadoAdminPage implements iVerdureMikadoLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new VerdureMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iVerdureMikadoRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: VerdureMikadoMetaBoxes
...
/*
   Class: VerdureMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class VerdureMikadoMetaBox implements iVerdureMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $verdure_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: VerdureMikadoTaxonomyOptions
   A class that initializes VerdureMikado Taxonomy Options
*/
class VerdureMikadoTaxonomyOptions {
	private static $instance;
...
/*
   Class: VerdureMikadoTaxonomyOption
   A class that initializes VerdureMikado Taxonomy Option
*/
class VerdureMikadoTaxonomyOption implements iVerdureMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new VerdureMikadoTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: VerdureMikadoUserOptions
   A class that initializes VerdureMikado User Options
*/
class VerdureMikadoUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: VerdureMikadoUserOption
   A class that initializes VerdureMikado User Option
*/
class VerdureMikadoUserOption implements iVerdureMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new VerdureMikadoUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: VerdureMikadoDashboardOptions
   A class that initializes VerdureMikado Dashboard Options
*/
class VerdureMikadoDashboardOptions {
	private static $instance;
...
/*
   Class: VerdureMikadoDashboardOption
   A class that initializes VerdureMikado Dashboard Option
*/
class VerdureMikadoDashboardOption implements iVerdureMikadoLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new VerdureMikadoDashboardFieldFactory();
	}
...
if ( ! function_exists( 'verdure_mikado_init_framework_variable' ) ) {
	function verdure_mikado_init_framework_variable() {
		global $verdure_mikado_Framework;
		
		$verdure_mikado_Framework = VerdureMikadoFramework::get_instance();
		$mkdSkinManager                 = new VerdureMikadoSkinManager();
		$verdure_mikado_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'verdure_mikado_before_options_map', 'verdure_mikado_init_framework_variable' );
}
?>